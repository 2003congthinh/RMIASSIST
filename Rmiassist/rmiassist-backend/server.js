...
const uri = "mongodb+srv://hieule0301:hieule0301@cluster02703.syx3xrh.mongodb.net/student-assistant?retryWrites=true&w=majority"
...
expressApp.post("/items", async (req, res) => {
    try {
        const foundStudents = await students.find({ s_id: req.body.s_id })
        foundStudents[0].items.length !== 0 ? res.send(foundStudents[0].items.map((ele) => { return JSON.parse(ele) })) : res.send([])
    } catch (error) {
        console.log(error)
    }
})
...
expressApp.post("/addItem", async (req, res) => {
    try {
        ...
        const foundStudents = await students.findOneAndUpdate(
            { s_id: req.body.s_id },
            { $push: { items: { $each: ["{\"title\": \"" + req.body.title + "\",\"startDate\": \"" + req.body.startDate + "\",\"endDate\": \"" + req.body.endDate + "\",\"location\": \"" + req.body.location + "\",\"color\": \"" + req.body.color + "\"}"] } } }
        )
...
expressApp.post('/groups', async (req, res) => {
    ...
    try {
        ...
        if (studentGroupCount >= 4) {
            return res.status(400).json({ error: 'Maximum number of groups reached for this student' });
        }
        ...
        if (groupMemberCount >= 5) {
            return res.status(400).json({ error: 'Maximum number of students reached for this group' });
        }
...
expressApp.listen(8000, () => console.log("Express Server started on port 8000"));
